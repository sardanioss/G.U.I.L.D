...
class conversion:
    ...
    def picture(element, elements, file_id, token, gen_dir):
        ...
        bg = "#FFFFFF"
        for ele in elements:
            if ele["name"] == "Picture_bg_"+element["name"][-1:]:
    ...
    def menubutton(element, elements):
        ...
        lines.extend(['\n\n# MenuBox',
                      f'\nMenuBox_{count_4} = tk.Menubutton(window, text="{text}", font=("{font}", {fontSize}), bg="{element_color}", fg="{color}")',
    ...
    def text(element, elements):
        ...
        for color in elements:
            if "color_"+suffix in color["name"]:
        ...
        if align == "none":
            lines.extend(['\n\n# Text'
                          f'\ntk.Label(window, text="{text}", font=("{font}", int({fontSize})), bg="{bg}",fg="{color}").place(x={x}, y={y})'])
        else:
            lines.extend(['\n\n# Text'
                          f'\ntk.Label(window, text="{text}", font=("{font}", int({fontSize})), justify="{align}", bg="{bg}",fg="{color}").place(x={x}, y={y})'])
    ...
    def button(element, elements, token, file_id, gen_dir):
        ...
        if Type == 'NR':
            ...
            lines.extend(['\n\n# Button',
                          f'button_{count_3} = tk.Button(window, text="{text}", bg="{color}", fg="{text_color}", relief="flat", command=lambda:[placeholder()], font=("{font}", {fontsize}))',
        ...
        if Type == 'R':
            ...
            lines.extend(['\n\n# Button',
                          ...
                          f'button_{count_3} = tk.Button(window, image = pic_b_{count_3}, relief="flat", command=lambda:[placeholder()])',
                          f'button_{count_3}.place(x={x}, y={y})'])
    ...
    def main():  # sourcery no-metrics
        ...
        for element in window_elements:
            ...
            elif "Picture" in element["name"] and "bg" not in element["name"]:
...
if __name__ == '__main__':
    ...
    img = Image.open("background.png")
    bg_image = ImageTk.PhotoImage(img)
    tk.Label(window, image=bg_image, borderwidth=0,
             relief="solid").place(x=0, y=0)

    # Frame
    tk.Frame(window, bg="#ffffff", width=900, height=420).place(x=50, y=47)

    # Text
    tk.Label(window, text="GUI", font=("Comic Sans MS", int(23)),
             anchor="w", bg="#FFFFFF", fg="#000000").place(x=58, y=47)

    # Text
    tk.Label(window, text="LD", font=("Comic Sans MS", int(23)),
             anchor="w", bg="#FFFFFF", fg="#2b2bff").place(x=121, y=47)

    # Text
    tk.Label(window, text="WELCOME TO\nGRAPHICAL USER INTERFACE\nLAYOUT DESIGNER", font=(
        "Roboto", int(19)), justify="left", bg="#FFFFFF", fg="#2e313b").place(x=96, y=142)

    # Text
    tk.Label(window, text="Build Eye pleasing GUI’s under minutes with \nsimple drag and drop interface for your \nproject without any hastle of debugging and \nmodifying code again and again!", font=(
        "Courier New", int(11)), justify="left", bg="#FFFFFF", fg="#2e313b").place(x=96, y=268)

    # Picutre (image should be in same folder as this file)
    img_0 = Image.open("picture_0.png")
    pic_0 = ImageTk.PhotoImage(img_0)
    tk.Label(window, image=pic_0, bg="#FFFFFF",
             borderwidth=0, relief="solid").place(x=564, y=99)

    # Line
    tk.Frame(window, width=1, height=332, bg="#000000").place(x=525, y=88)

    # Text
    tk.Label(window, text="Token ID:", font=("Roboto", int(15)),
             justify="left", bg="#3f3f3f", fg="#7e7e7e").place(x=589, y=141)

    # Text
    tk.Label(window, text="File  Link:", font=("Roboto", int(15)),
             justify="left", bg="#3f3f3f", fg="#7e7e7e").place(x=589, y=218)

    # Entrybox
    entrybox_0 = tk.Text(window, width=18, height=1)
    entrybox_0.place(x=706, y=144)

    # Entrybox
    entrybox_1 = tk.Text(window, width=18, height=1)
    entrybox_1.place(x=706, y=220)

    # Button
    button_0 = tk.Button(window, text="Browse Files", bg="#01d562", fg="#ffffff",
                         relief="flat", command=lambda: [conversion.main()], font=("Roboto", 15))
    button_0.place(x=663, y=317)
